                ...
                <div class="row ">
                    ...
                    <div class="col-lg-6 bg-light p-4" style="border-left:5px solid lightblue">
                        <form method="POST" enctype="multipart/form-data">
                            <center><h3 class="font-weight-normal">Edit Product Detail's</h3></center>
                            <hr/>
                            ...
                            <input type="file" value="<?php echo $img_address;?>" class="btn btn-light mt-4"  name="my_file" id="my_file"accept="image/*"><br>
                            
                            <label class="mt-2" for="description" >Product Description </label><small style="color:red"> * <?php echo $description_err?></small>
                            <textarea rows="6" cols="71" value="12323213" name="description" class="form-control" id = "description"style="resize:horizontally;"></textarea>
    ...
    if(isset($_POST["add_btn"])) {
        if($error == false){
            if($_FILES["my_file"]["name"] == "") {
                ...
                $sql = "UPDATE `product` SET `product_name`= '$name', `product_short_intro`= '$intro',
                `product_location` = '$city', `product_price` = '$price', `product_quantity` = '$quantity',
                `product_img` = '$img_address', `product_description` = '$description' WHERE PID  = $pid";
        
                if($conn->query($sql) === true){
                    echo "<script> successAlert('success','Product Updated successfully');</script>";
                    echo "<script>window.location.href = 'seller_view_all_products.php'; </script>";
                }
                else{
                    echo "<script>warningAlert('warning','Error in products updation.');</script>";
                }
            }
        }
    }
